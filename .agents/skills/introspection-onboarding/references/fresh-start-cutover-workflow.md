...
- Require fresh, resume, concurrent-project, workspace-change, end, and non-Git scenarios wherever the native producer exposes those lifecycle transitions.
- Verify each scenario through the managed adapter, canonical runtime, context ledger, source activity, canonical activity version, canonical outbox event, remote event ID, and dashboard source-time query.
