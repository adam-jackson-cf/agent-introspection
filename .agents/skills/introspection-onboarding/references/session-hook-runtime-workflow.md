...
Install one shared session-context runtime and only the thin producer adapters and proxy components required by approved producer integrations at a stable versioned managed location.
...
- Configure Codex app-server only through `.agents/skills/introspection-onboarding/scripts/adapters/codex-app-server/install.py`; the installer owns the managed proxy, app-server adapter, immutable real-executable record, and user LaunchAgent `CODEX_CLI_PATH` override. Do not manually invoke the proxy or adapter.
